...
if __debug__:
    ...
    ENABLE_VCP_IFACE = True or utils.EMULATOR or utils.BITCOIN_ONLY
    if ENABLE_VCP_IFACE:
        # interface used for cdc/vcp console emulation (debug messages)
        iface_vcp = io.VCP(
            iface_num=2 if True else 3,
            data_iface_num=3 if True else 4,
            ep_in=0x83 if True else 0x84,
            ep_out=0x03 if True else 0x04,
            ep_cmd=0x84 if True else 0x85,
        )
